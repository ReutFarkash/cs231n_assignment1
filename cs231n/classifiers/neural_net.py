...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    # shape (N, C).                                                             #
    #############################################################################
    H1 = np.dot(X, W1) + b1  # first hidden layer
    R1 = np.maximum(H1, 0)  # activation Relu
    scores = np.dot(R1, W2) + b2  # second and last hidden layer
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    #############################################################################
    scores[np.arange(N), y] -= 1  # [scores]=num_inputs x num_classes=N x C
    scores /= N
    grads['b2'] = np.sum(scores, axis=0)  # [b2]=C
    grads['W2'] = np.dot(R1.T, scores) + 2*reg*W2  # [W2]=H x C
    dH1 = np.dot(scores, W2.T)  # derivation wrt relu input
    dH1[R1 <= 0] = 0
